...
public class Warehouse {
    ...
    public void receiveShipment(String productId, int amount) {
        ...
        synchronized (p) { p.increaseQuantity(amount); }
    }
    ...
    private Product getProductById(String id) {
        ...
        try {
            ...
            if (p == null) throw new NoSuchElementException("No product with ID: " + id);
            return p;
        } finally { rwLock.readLock().unlock(); }
    }
    ...
    public void saveTo(Path file) throws Exception {
        rwLock.readLock().lock();
        try { InventoryPersistence.saveToFile(products, file); }
        finally { rwLock.readLock().unlock(); }
    }
